...
final class RingSlotPresentationTests: XCTestCase {
    ...
    /// `QuotaMetric` reports "no data" as a negative percentage. Clamping it
    ...
    func testUnknownQuotaIsUnavailableRatherThanMeasured() {
        ...
        XCTAssertEqual(unknown.captionText, "quota.state.unavailable".localized())
    ...
    func testUnlimitedMetricWithUsageShowsUsageNotUnused() {
        ...
        XCTAssertNotEqual(onDemand.heroText, "quota.state.unused".localized())
    }

    func testGenuinelyUnusedWindowStillReadsUnused() {
        let untouched = slot(percentage: 100, usage: nil)
        XCTAssertEqual(untouched.heroText, "quota.state.unused".localized())
    }
    ...
    func testCaptionUsesRawIdentityNotDisplayName() {
        ...
        XCTAssertEqual(
            slot(rawName: "five-hour-session", percentage: 50).windowCaption,
            String(format: "quota.window.hours".localized(), 5)
        )
    ...
    func testMetricWithUsageAndNoLimitIsUnlimited() {
        let onDemand = QuotaMetric(
            name: "on-demand", percentage: 100, resetTime: "", used: 123, limit: nil)
    ...
    func testMetricWithARealLimitIsNotUnlimited() {
        let capped = QuotaMetric(
            name: "plan-usage", percentage: 40, resetTime: "", used: 60, limit: 100)
        XCTAssertFalse(capped.isUnlimitedUsage)
    }

    func testUntouchedMetricIsNotUnlimited() {
        let untouched = QuotaMetric(
            name: "on-demand", percentage: 100, resetTime: "", used: 0, limit: nil)
        XCTAssertFalse(
            untouched.isUnlimitedUsage, "Nothing used means \"unused\" is still the honest label")
    }
    ...
    func testStandaloneMetricShowsItsValueDespiteTheSentinel() {
        ...
        XCTAssertNotEqual(balance.captionText, "quota.state.unavailable".localized())
    }

    func testNonStandaloneSentinelIsStillUnavailable() {
        let noData = slot(percentage: -1, usage: nil)
        XCTAssertNil(noData.heroText)
        XCTAssertEqual(noData.captionText, "quota.state.unavailable".localized())
    }
    ...
    func testRowsAreCappedAtFourColumns() {
        let arranged = RingSlotArrangement.arrange(Array(1...9), rawName: { _ in "" })
        for row in RingSlotArrangement.rows(arranged) {
            XCTAssertLessThanOrEqual(row.count, RingSlotArrangement.maxColumns)
        }
    }

    func testShortRowsStillReserveTheMinimumColumns() throws {
        let rows = RingSlotArrangement.rows(RingSlotArrangement.arrange(["only"], rawName: { $0 }))
        let row = try XCTUnwrap(rows.first)
        XCTAssertEqual(RingSlotArrangement.columnCount(for: row), 3)
    }

    // MARK: - Shared arrangement (dashboard and menu must agree)

    func testWeeklyWithoutSessionLeavesTheSessionColumnEmpty() {
        let arranged = RingSlotArrangement.arrange(
            ["seven-day-weekly", "extra-usage"], rawName: { $0 })
        XCTAssertNil(
            arranged[0], "The Session column stays empty rather than being filled by Weekly")
    ...
    func testProvidersWithoutRolesAreNotPaddedWithEmptyColumns() {
        let arranged = RingSlotArrangement.arrange(
            ["copilot-chat", "copilot-premium"], rawName: { $0 })
    ...
    func testAccessibilityDescriptionReportsUnavailableRatherThanZero() {
        ...
        XCTAssertTrue(
            description.contains("quota.state.unavailable".localized()), description)
