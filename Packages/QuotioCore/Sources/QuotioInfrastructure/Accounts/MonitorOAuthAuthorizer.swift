...
public actor MonitorOAuthAuthorizer: OAuthAuthorizing {
    public typealias KiroIdentityResolver = @Sendable (
    ...
    public func completeManualCode(
        ...
        guard providerID.rawValue == QuotaProvider.claude.rawValue else {
            throw OAuthFlowFailure.unsupportedProvider
        }
    ...
    private func browserConfiguration(_ provider: QuotaProvider) throws -> BrowserConfiguration {
        switch provider {
        case .codex:
    ...
    private func saveBrowserCredential(
        ...
        var email = OAuthIdentity.jwtString(idToken, claim: "email")
        var accountID: String?
        if provider == .codex {
            accountID = OAuthIdentity.jwtNestedString(
                idToken,
        ...
        let key = email ?? accountID ?? "\(provider.oauthDisplayName) \(UUID().uuidString.prefix(8))"
        let account = Account.oauth(
            provider: provider,
            accountKey: key,
    ...
    nonisolated static func kiroAccountKey(identity: String?, clientID: String) -> String {
        ...
        return "AWS Builder ID • " + String(OAuthIdentity.fingerprint(clientID).prefix(8))
    }
