...
struct YubiKeySettingsSection: View {
    ...
    private var actionsView: some View {
        ...
        case .active:
            ...
            if model.snapshot.identities.count > 1 {
                identityPicker
                Button("settings.yubikey.change".localized()) {
                    Task { await select() }
                }
        ...
        case .notConfigured:
            if !model.snapshot.identities.isEmpty {
                identityPicker
                Button("settings.yubikey.use".localized()) {
                    Task { await select() }
                }
                    .disabled(selectedID.isEmpty)
            }
            ...
            if !model.snapshot.devices.isEmpty {
                ...
                Button("settings.yubikey.setup".localized()) {
                    provisioningDevice = model.snapshot.devices.first { $0.id == selectedDeviceID }
                }
                .disabled(selectedDeviceID.isEmpty)
            }

            if model.snapshot.identities.isEmpty, model.snapshot.devices.isEmpty {
                Label(
                    model.snapshot.isConnected
                        ? "settings.yubikey.detected".localized()
                        : "settings.yubikey.insert".localized(),
                    systemImage: model.snapshot.isConnected ? "key.badge.exclamationmark" : "key.slash"
                )
                .foregroundStyle(.secondary)
            }
        }
    ...
    private var identityPicker: some View {
        Picker("settings.yubikey.identityPicker".localized(), selection: $selectedID) {
            ForEach(model.snapshot.identities) { identity in
    ...
    private func select() async {
        guard let identity = model.snapshot.identities.first(where: { $0.id == selectedID }) else { return }
        if await model.select(identity) {
    ...
    private func completeProvisioning() async {
        switch await model.completeProvisioning() {
    ...
    private func synchronizeSelections() {
        if selectedDeviceID.isEmpty {
            selectedDeviceID = model.snapshot.devices.first?.id ?? ""
        }
        if let selectedIdentity = model.snapshot.selectedIdentity {
            selectedID = selectedIdentity.id
        } else if selectedID.isEmpty {
            selectedID = model.snapshot.identities.first?.id ?? ""
        }
    }
}
