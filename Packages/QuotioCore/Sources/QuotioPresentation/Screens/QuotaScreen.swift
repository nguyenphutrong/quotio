...
private struct RingSlotLabel: View {
    ...
    var body: some View {
        let statusColor = slot.isUnknown
            ? Color.secondary
            : displayHelper.statusColor(remainingPercent: slot.remainingPercent)
...
private struct RingSlotBody: View {
    ...
    var body: some View {
        ...
        HStack(spacing: 14) {
            ZStack {
                RingProgressView(
                    percent: slot.isUnknown ? RingProgressView.unknownPercent : displayPercent,
                    size: 64,
                    lineWidth: 9,
                    tint: statusColor,
                    showLabel: false
                ...
                // `RingProgressView` is drawn with `showLabel: false` so the
                // number can be sized for this layout, so the unknown
                // placeholder has to be rendered here.
                Text(slot.isUnknown ? "—" : "\(Int(displayPercent))")
                    .font(.system(size: 14, weight: .medium))
                    .foregroundStyle(slot.isUnknown ? Color.secondary : statusColor)
                    .monospacedDigit()
            }
...
private struct UsageRowV2: View {
    ...
    /// See `QuotaMetric.isStandaloneMetric`.
    ...
    var body: some View {
        let displayPercent = displayHelper.displayPercent(remainingPercent: remainingPercent)
        let statusColor = isUnknown
            ? Color.secondary
            : displayHelper.statusColor(remainingPercent: remainingPercent)
