...
private struct MenuAccountCardView: View {
    ...
    private var quotaContentSection: some View {
        let showsFable = provider == .claude && settings.showClaudeFableWeekly
        let quotaModels = data.models.filter {
            showsFable || provider != .claude || $0.name != "seven-day-fable"
        }
        ...
        let models: [ModelBadgeData] = {
            if isAntigravity {
                ...
                var meterModels = quotaModels.filter { !$0.isStandaloneMetric }.map {
                    ModelBadgeData(name: showsFable && $0.name == "five-hour-session"
                                   ? "quota.metric.fiveHourCompact".localized() : $0.displayName,
                                   percentage: $0.percentage, resetTime: $0.resetTime)
                }
                // Keep the three-column grid stable when Claude has not reported Fable yet.
                if isCardStyle && showsFable && !quotaModels.contains(where: { $0.name == "seven-day-fable" }) {
    ...
    private func quotaLayout(models: [ModelBadgeData]) -> some View {
        ...
        case .card:
            CardGridLayout(models: models, displayMode: settings.quotaDisplayMode,
                           showsResetBelow: provider == .claude)
        }
    }
    ...
    private var primaryResetModel: QuotaMetric? {
        ...
        let validModels = data.models.filter { model in
            guard provider != .claude || settings.showClaudeFableWeekly || model.name != "seven-day-fable"
            else { return false }
...
private struct CardGridLayout: View {
    let models: [ModelBadgeData]
    let displayMode: QuotaDisplayMode
    /// Three columns keep the Claude 5h / Weekly / Fable row on one line.
    var columnCount: Int = 2
    /// Claude has no room for an inline reset time beside a narrow card, so it
    /// goes under the bar instead and the card drops its tooltip.
        ...
        LazyVGrid(columns: columns, spacing: 8) {
            ForEach(models, id: \.name) { (model: ModelBadgeData) in
                VStack(alignment: .trailing, spacing: 4) {
                    ...
                    .background(Color.secondary.opacity(0.05))
                    .clipShape(RoundedRectangle(cornerRadius: 8))
                    .menuNativeTooltip(showsResetBelow
                                       ? "" : model.name + (model.formattedResetTime.map { " · " + $0 } ?? ""))
                    if showsResetBelow {
