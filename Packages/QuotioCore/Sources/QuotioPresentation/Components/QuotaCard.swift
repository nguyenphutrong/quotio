...
struct QuotaCard: View {
    ...
    let accounts: [ManagedAuthFile]
    var quotaData: [String: ProviderQuota]?
        ...
        VStack(spacing: 12) {
            ForEach(Array(aggregatedModels.keys.sorted()), id: \.self) { modelName in
                if let data = aggregatedModels[modelName] {
                    let displayName = QuotaMetric(name: modelName, percentage: 0.0, resetTime: "").displayName
                    QuotaSection(
                        title: displayName,
...
#Preview {
    ...
    let mockQuota: [String: ProviderQuota] = [
        "[email protected]": ProviderQuota(
            models: [
                QuotaMetric(name: "gemini-3-pro-high", percentage: 65.0, resetTime: "2025-12-25T00:00:00Z"),
                QuotaMetric(name: "gemini-3-flash", percentage: 80.0, resetTime: "2025-12-25T00:00:00Z"),
                QuotaMetric(name: "claude-sonnet-4-5-thinking", percentage: 45.0, resetTime: "2025-12-25T00:00:00Z")
            ]
        )
    ]
