...
public enum ModelCatalogFreshness: Equatable, Sendable {
...
public struct ModelCatalogState: Equatable, Sendable {
    ...
    public private(set) var entries: [ModelCatalogEntry] = []

    public private(set) var freshness: ModelCatalogFreshness = .never

    /// A fetch is in flight.
    public private(set) var isLoading = false

    /// The most recent fetch attempt failed.
    public private(set) var lastFetchFailed = false
    ...
    public private(set) var hasCompletedFetch = false

    /// The proxy answered successfully and listed no models at all.
    public var isEmptyLiveResult: Bool {
    ...
    public mutating func apply(entries: [ModelCatalogEntry], fetchedAt: Date) {
...
public enum ModelCatalog {
    ...
    public static func parse(_ data: Data) throws -> [ModelCatalogEntry] {
    ...
    public static func displayEntries(_ entries: [ModelCatalogEntry]) -> [ModelCatalogEntry] {
        var seen = Set<String>()
        return entries
    ...
    public static func agentSetupModels(from entries: [ModelCatalogEntry]) -> [AvailableModel] {
