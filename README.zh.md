...
CLIProxyAPI 控制面板（访问地址 `http://localhost:8317`）中显示的 `secret-key` 字段用于**远程管理认证**：

| 设置 | 说明 |
|------|------|
| `allow-remote` | 设为 `true` 时，允许从网络中的其他设备访问管理 API |
| `secret-key` | 远程 API 请求所需的认证令牌。默认自动生成 UUID |
| `disable-control-panel` | 设为 `true` 时，完全禁用 Web 控制面板 |

> **注意**：对于仅本地使用（默认配置），可以忽略 `secret-key` 字段。只有在需要从其他设备远程访问时才需要使用它。
