...
//  CKota - Agent configuration modal with automatic/manual modes
//
...
struct AgentConfigSheet: View {
    ...
    private func copyPreviewToClipboard(index: Int? = nil) {
        ...
        let content: String = if let idx = index, idx < config.rawConfigs.count {
            config.rawConfigs[idx].content
        } else {
            config.rawConfigs.map(\.content).joined(separator: "\n\n---\n\n")
        }
                    ...
                    HStack(spacing: 4) {
                        if viewModel.isConfiguring {
                            ...
                            Image(systemName: viewModel
                                .configurationMode == .automatic ? "gearshape.2" : "square.and.arrow.down")
                        }
                        Text(viewModel.configurationMode == .automatic ? "agents.apply".localized() : "agents.saveConfig"
                            .localized())
                    }
                }
                .buttonStyle(.borderedProminent)
...
private struct StorageOptionButton: View {
    ...
    private var displayName: String {
        switch option {
        case .jsonOnly: "agents.storage.jsonOnly".localized()
        case .shellOnly: "agents.storage.shellOnly".localized()
        case .both: "agents.storage.both".localized()
        }
    }
