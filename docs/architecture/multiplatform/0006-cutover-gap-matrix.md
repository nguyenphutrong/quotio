...
| Agents | Native macOS SwiftUI write path remains authoritative | Codex, OpenCode, and Factory Droid adapters support descriptor, detection, diff preview, install, backup, and rollback; other agents remain read-only or guide-only | Descriptor, detection, diff, install, backup, and rollback endpoints exist per OS |
