...
editing UI, or agent write/rollback parity.
...
| Settings | Native Swift settings remain authoritative | Credential Manager-backed bootstrap config and native `Quotio/*` credential write primitives; shared settings placeholder enabled, shared settings controls hidden | Credentials live in Keychain/Credential Manager and unsupported controls are hidden |
