...
automation. The Windows host now builds a native WinUI/WebView2 shell and
bridge with Credential Manager-backed bootstrap configuration, shared remote
credential editing, local crash-report capture with an optional upload endpoint,
native agent write adapters, and a Velopack installer/update package path. It
does not yet have production signing or production crash ingestion.
...
Windows builds keep unsigned ZIP artifacts for smoke testing and produce
...
| --- | --- | --- | --- |
| App shell | Production SwiftUI menu bar app | Native WinUI shell with WebView2 renderer | Windows tray, lifecycle, single-instance, window restore, and native dialogs verified |
...
| Runtime process | Existing CLIProxyAPI lifecycle plus shared Rust foundation | Host can start, stop, restart, health-gate startup, report crashed child exits, write local diagnostics, and capture/upload redacted crash reports when an upload endpoint is configured; shared Rust foundation is packaged with the host artifact and installer output | No-zombie-process and recovery checks pass in CI and manual OS smoke tests before production packaging |
...
| Packaging | Existing release path plus unsigned smoke artifact workflow | Unsigned ZIP smoke artifact plus Velopack installer artifacts from CI and manual release workflow | Signing, uninstall, upgrade, and user-data preservation tested |
...
- Run `bun run windows:gates:check` before changing Windows host routes or
  capability defaults.
- Run `cargo test`.
...
- Verify the Windows ZIP smoke artifact is uploaded from CI, and verify the zip
