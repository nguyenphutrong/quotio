...
export async function proxyStatus(port: number, ctx: CLIContext): Promise<CommandResult> {
  try {
    ...
    logger.print("");
    printProcessSection(managedRunning, healthy, managedPid, managedState, port);
    logger.print("");
