...
export async function proxyInstall(ctx: CLIContext): Promise<CommandResult> {
  try {
    ...
    if (existingInfo.exists && existingInfo.isExecutable) {
      if (ctx.format === "json") {
        logger.print(formatJson({ status: "already_installed", ...existingInfo }));
      } else {
      ...
      return { success: true, data: { status: "already_installed", ...existingInfo } };
    }
    ...
    if (!verification.valid) {
      return { success: false, message: `Installation failed: ${verification.error}` };
    }
