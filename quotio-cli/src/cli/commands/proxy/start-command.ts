...
export async function proxyStart(port: number, ctx: CLIContext): Promise<CommandResult> {
  try {
    ...
    if (running) {
      ...
      if (ctx.format === "json") {
        ...
        logger.print(`${colors.yellow("Proxy is already running")} on port ${state.port}`);
      }
